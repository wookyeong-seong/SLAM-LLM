...
def model_factory(train_config, model_config, **kwargs):
    ...
    processor = setup_encoder(train_config, model_config, **kwargs)
...
def setup_encoder(train_config, model_config, **kwargs):
    ...
    if len(encoder_list) == 1:
        ...
        if encoder_name == "qwen2-audio":
            encoder = AutoProcessor.from_pretrained(model_config.llm_path)
...
def setup_llm(train_config, model_config, **kwargs):
    ...
    if (train_config.enable_fsdp or train_config.enable_ddp) and train_config.low_cpu_fsdp:
        ...
        if rank == 0:
            ...
            elif "qwen2-audio" in model_config.llm_name.lower():
...
class slam_model(nn.Module):
    def __init__(
        ...
        **kwargs
        ...
        super().__init__()
        # modality encoder, which is Processor for qwen2-audio 
    ...
    def forward(self,
        ...
        final_labels, model_outputs = self.llm(input_ids=input_ids,
        ...
        if self.metric:
            with torch.no_grad():
                ...
                #print(f"input_ids: {input_ids.size()}, preds: {preds.size()}, atten_mask: {attention_mask.size()}, feat_attn_mask: {feature_attention_mask.size()}, final_labels: {final_labels.size()}", flush=True)
                #acc = compute_accuracy(preds.detach()[:, :-1], labels.detach()[:, 1:], ignore_label=-100)
                acc = compute_accuracy(preds.detach()[:, :-1], final_labels.detach()[:, 1:], ignore_label=-100)
    ...
    def generate(self,
        ...
        # beam search
        '''
        model_outputs = self.llm.generate(input_ids=input_ids,
